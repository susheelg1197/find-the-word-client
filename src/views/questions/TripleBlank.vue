...
<div class="multiple-question quiz-answers container-fluid">
   <div class="quiz-answer col-md-4">
    ...
    toggleAnswer(index) {
      if (this.answers.includes(index)) {
        ...
        if (index<3){
          ...
          this.answers=this.answers.filter(item=>item<3 || item>5)
          this.answers.push(index);
        }else if(index>5){
          this.answers=this.answers.filter(item=>item<6)
          this.answers.push(index);
        }
      }
    },
